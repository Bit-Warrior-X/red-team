...
class ReportGenerator:
    ...
    def _generate_markdown(self, results: ScanResult, vulns: list[Vulnerability]):
        ...
        params = aggregate_parameters(vulns, results.urls_scan_surface)
        lines  = [f"# RedScanner Report — {results.target}", ""]

        if self.config.plan_title:
            lines += [f"**Engagement:** {self.config.plan_title}", ""]
        if self.config.plan_description:
            lines += [self.config.plan_description, ""]
        ...
        if self.config.strict_domain_reports:
            lines += [
                "**Domain scope:** Vulnerabilities section only includes findings on the target or its subdomains.",
                "",
            ]
        ...
        lines += [
            ...
            f"- **Full scan URL list:** {len(results.urls_scan_surface)}",
            "",
        ]
        ...
        for a in results.assets_discovered[:LIST_CAP]:
            ...
            lines.append(f"| {url_cell} | {a.ip or ''} | {port} | {a.status_code or ''} | {title} | {tech} |")
        ...
        if params:
            lines += ["", "### Query parameters observed", "",
        ...
        for i, v in enumerate(vulns, 1):
            ...
            if v.cvss_score is not None:
                lines.append(f"- **CVSS v3.1:** {v.cvss_score:.1f}")
    ...
    def _generate_html(self, results: ScanResult, vulns: list[Vulnerability]):
        ...
        # Vulnerability rows — v0.3 adds CVSS column
        vuln_rows = ""
        for v in vulns:
            color = SEVERITY_COLORS.get(v.severity, "#6b7280")
            cvss_cell = (
                f'<span style="font-weight:700;color:{color}">{v.cvss_score:.1f}</span>'
                if v.cvss_score is not None else
            ...
            <tr>
                ...
                <td style="font-size:13px;white-space:nowrap">{cvss_cell}</td>
                <td>{html.escape(str(v.vuln_type))}</td>
                <td style="word-break:break-all;max-width:280px;font-size:13px">{self._html_url_link(v.url)}</td>
        ...
        if self.config.plan_title or self.config.plan_description or self.config.methodology_frameworks:
            parts = [
                "<div style='margin:12px 0 20px;padding:16px;background:#0f172a;"
                "border:1px solid #334155;border-radius:8px;font-size:13px;line-height:1.5'>"
            ]
            if self.config.plan_title:
                parts.append(
                    f"<div style='font-weight:700;color:#f1f5f9;margin-bottom:8px'>"
                    f"{html.escape(self.config.plan_title)}</div>"
                )
            if self.config.plan_description:
                parts.append(
                    f"<p style='margin:0 0 12px;color:#94a3b8'>"
                    f"{html.escape(self.config.plan_description)}</p>"
                )
            if self.config.methodology_frameworks:
        ...
        for sev in ["critical", "high", "medium", "low", "info"]:
            ...
            summary_cards += (
                f'<div style="text-align:center;padding:16px 24px;background:{c}11;'
                f'border:1px solid {c}33;border-radius:8px">'
                f'<div style="font-size:28px;font-weight:800;color:{c}">{count}</div>'
                f'<div style="font-size:12px;color:{c};text-transform:uppercase;font-weight:600">{sev}</div>'
                f'</div>'
            )

        def table_wrap(title: str, inner: str) -> str:
            return (
                f'<h2 style="color:#f1f5f9;font-size:18px;margin:28px 0 12px">'
                f'{html.escape(title)}</h2>'
                f'<table>{inner}</table>'
            )

        dom_rows = "".join(
            f"<tr><td style='word-break:break-all'>{html.escape(d)}</td></tr>"
            for d in results.domains[:LIST_CAP]
        )
        domains_tbl = table_wrap(
            f"Domains ({len(results.domains)})",
            f"<tr><th>Hostname</th></tr>{dom_rows}",
        )

        port_rows = "".join(
            f"<tr><td>{html.escape(str(row.get('host','')))}</td>"
            f"<td>{row.get('port','')}</td>"
            f"<td>{html.escape(str(row.get('protocol','tcp')))}</td></tr>"
            for row in results.open_ports[:LIST_CAP]
        )
        ports_tbl = table_wrap(
            f"Open ports ({len(results.open_ports)})",
            f"<tr><th>Host</th><th>Port</th><th>Protocol</th></tr>{port_rows}"
            if port_rows else "<tr><td colspan='3'>No open ports recorded (run naabu module).</td></tr>",
        )
        ...
        for a in results.assets_discovered[:LIST_CAP]:
            ...
            asset_rows += (
                f"<tr>"
                f"<td style='word-break:break-all;font-size:12px'>{self._html_url_link(a.url)}</td>"
                f"<td>{html.escape(str(a.ip or ''))}</td>"
                f"<td>{a.port if a.port is not None else ''}</td>"
                f"<td>{a.status_code if a.status_code is not None else ''}</td>"
                f"<td style='max-width:200px;font-size:12px'>{html.escape((a.title or '')[:80])}</td>"
                f"<td style='font-size:11px;max-width:220px'>{tech}</td>"
                f"<td>{'yes' if a.alive else 'no'}</td>"
                f"</tr>"
            )
        assets_tbl = table_wrap(
            f"HTTP services / assets ({len(results.assets_discovered)})",
            f"<tr><th>URL</th><th>IP</th><th>Port</th><th>HTTP</th><th>Title</th><th>Tech</th><th>Alive</th></tr>{asset_rows}"
            if asset_rows else "<tr><td colspan='7'>No httpx assets.</td></tr>",
        )

        def url_list_rows(urls: list[str]) -> str:
            return "".join(
                f"<tr><td style='word-break:break-all;font-size:12px'>{self._html_url_link(u)}</td></tr>"
                for u in urls[:LIST_CAP]
            )

        crawl_tbl = table_wrap(
            f"Crawled URLs ({len(results.urls_crawled)})",
            f"<tr><th>URL</th></tr>{url_list_rows(results.urls_crawled)}"
            if results.urls_crawled else "<tr><td>No crawl output.</td></tr>",
        )
        hist_tbl = table_wrap(
            f"Historical URLs — wayback/gau ({len(results.urls_historical)})",
            f"<tr><th>URL</th></tr>{url_list_rows(results.urls_historical)}"
            if results.urls_historical else "<tr><td>No historical URLs collected.</td></tr>",
        )
        scope_tbl = table_wrap(
            f"Scanner target URL list ({len(results.urls_scan_surface)})",
            f"<tr><th>URL</th></tr>{url_list_rows(results.urls_scan_surface)}"
            if results.urls_scan_surface else "<tr><td>No URLs in scope.</td></tr>",
        )

        params     = aggregate_parameters(vulns, results.urls_scan_surface)
        param_rows = ""
        for p in params[:LIST_CAP]:
            ex    = ", ".join(self._html_url_link(x) for x in p["example_urls"][:4])
            tools = html.escape(", ".join(p.get("tools") or []))
            param_rows += (
                f"<tr><td><code>{html.escape(p['name'])}</code></td>"
                f"<td>{p['occurrences']}</td>"
                f"<td style='word-break:break-all;font-size:11px'>{ex}</td>"
                f"<td>{tools}</td></tr>"
            )
        params_tbl = table_wrap(
            f"Query parameters observed ({len(params)})",
            f"<tr><th>Parameter</th><th>Count</th><th>Example URLs</th><th>Tools</th></tr>{param_rows}"
            if param_rows else "<tr><td colspan='4'>No query parameters extracted.</td></tr>",
        )

        surface_block = (
            f"<div style='margin:24px 0'>"
            f"{domains_tbl}{ports_tbl}{assets_tbl}"
            f"{crawl_tbl}{hist_tbl}{scope_tbl}{params_tbl}"
            f"</div>"
        )
        ...
        if self.config.strict_domain_reports:
            strict_note = (
                ...
                "Vulnerabilities table only includes findings on the target or its subdomains.</p>"
            )
...
<strong>Duration:</strong> {html.escape(duration)}</p>
<p class="meta"><strong>Modules:</strong> {html.escape(', '.join(results.modules_run))} &nbsp;
<strong>HTTP assets:</strong> {len(results.assets_discovered)} &nbsp;
<strong>Findings:</strong> {len(vulns)}</p>
{strict_note}
{plan_ctx_html}
...
<table>
<tr><th>Severity</th><th>CVSS&nbsp;v3.1</th><th>Type</th><th>URL</th><th>Param</th><th>Tool</th><th>Description</th></tr>
{vuln_rows}
</table>
